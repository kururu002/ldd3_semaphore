...
char * get_data_entry(struct scull_qset* head, int index, int create)
{
    int i;
    for (i=0; i< index; i++)
    {
...
ssize_t scull_read(struct file *pfile, char __user *pbuff, size_t num, loff_t *ploff)
{
    ...
    {
        char *entry = NULL;
        entry = get_data_entry(pdev->head, pdev->pos, 0);
        if (entry == NULL)
            goto out;
        if (copy_to_user(pbuff, entry + *ploff, num) != 0)
        {
            printk("copy error!\n");
            goto out;
        }
...
ssize_t scull_write(struct file *pfile, const char __user *pbuff, size_t num, loff_t *ploff){
     ...
     */
    entry = get_data_entry(pdev->head, pdev->pos, 1);
    if (entry)
        ClearPageReserved(virt_to_page(entry));
    else
        goto out;
    free_page(entry);
    entry = (char*)__get_free_page(GFP_KERNEL);
    if (copy_from_user(entry, pbuff, num) != 0)
    {
        printk("copy error!\n");
        goto out;
    }
...
static int scull_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
    ...
    char *entry = get_data_entry(pdev->head, pdev->pos, 0);
    if (entry ==NULL)
        // Find a proper number
        return -1;
    ppage = virt_to_page(entry);
    get_page(ppage);
    vmf->page = ppage;
        return 0;
}
...
static int scull_map(struct file *pfile, struct vm_area_struct *vma)
{
        ...
        remap_pfn_range(vma, vma->vm_start, (__pa(entry)) >> PAGE_SHIFT, vma->vm_end - vma->vm_start, vma->vm_page_prot);
